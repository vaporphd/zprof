...
8. **Emit `iosApp/` skeleton via `xcodegen` when available, else write the `iosApp.xcodeproj` bundle by hand.** Include `Podfile` if Q8 = Pods. **The generated xcodeproj MUST wire `FRAMEWORK_SEARCH_PATHS`** so Swift `import shared` resolves in Xcode/SourceKit against the framework the doctor pass links at step 10. Two acceptable shapes:
    - **xcconfig shape (preferred when xcodegen is available):** emit `iosApp/Configs/shared.xcconfig` containing
      ```
      FRAMEWORK_SEARCH_PATHS = $(inherited) $(SRCROOT)/../shared/build/bin/$(KONAN_TARGET)/debugFramework $(SRCROOT)/../shared/build/xcode-frameworks/$(CONFIGURATION)/$(SDK_NAME)
...
- Never emit `iosApp.xcodeproj` without `FRAMEWORK_SEARCH_PATHS` wired to `$(SRCROOT)/../shared/build/bin/$(KONAN_TARGET)/debugFramework` and the KMP-official `embedAndSignAppleFrameworkForXcode` Gradle task hooked into an Xcode Run Script build phase (§4 step 8). The framework binary on disk is a necessary but not sufficient condition for SourceKit to resolve `import shared`; the xcodeproj must also know where to look. **Reason:** shakedown-4 (2026-07-19) F-7 discovered that even with `linkDebugFrameworkIosSimulatorArm64` green (F-6 fix intact), Swift `import shared` still reported `No such module 'shared'` because the xcodeproj lacked the search path. Linking without wiring is a half-fix.
