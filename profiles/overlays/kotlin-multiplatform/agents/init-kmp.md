...
8. **Emit `iosApp/` skeleton via `xcodegen` when available, else write the `iosApp.xcodeproj` bundle by hand.** Include `Podfile` if Q8 = Pods. **The generated xcodeproj MUST wire `FRAMEWORK_SEARCH_PATHS`** so Swift `import shared` resolves in Xcode/SourceKit against the framework the doctor pass links at step 10. Two acceptable shapes:
    - **xcconfig shape (preferred when xcodegen is available):** emit `iosApp/Configs/shared.xcconfig` containing
      ```
      // Default (device + universal fallback) — Xcode-staged framework path only
      FRAMEWORK_SEARCH_PATHS = $(inherited) $(SRCROOT)/../shared/build/xcode-frameworks/$(CONFIGURATION)/$(SDK_NAME)
      // Simulator arm64 (Apple Silicon Mac running iOS Simulator)
...
- Never emit `iosApp.xcodeproj` without `FRAMEWORK_SEARCH_PATHS` wired via per-SDK/arch conditionals (see §4 step 8 xcconfig template) AND the KMP-official `embedAndSignAppleFrameworkForXcode` Gradle task hooked into an Xcode Run Script build phase. The framework binary on disk is a necessary but not sufficient condition for SourceKit to resolve `import shared`; the xcodeproj must also know where to look, using Xcode-valid variables. **Reason:** shakedown-4 (2026-07-19) F-7 discovered that even with `linkDebugFrameworkIosSimulatorArm64` green (F-6 fix intact), Swift `import shared` still reported `No such module 'shared'` because the xcodeproj lacked the search path. Linking without wiring is a half-fix. **Reason (extended):** shakedown-5 (2026-07-19) F-12 discovered the initial fix used `$(KONAN_TARGET)` in the xcconfig — a Kotlin/Native Gradle variable, NOT an Xcode build setting. Xcode leaves it un-substituted and SourceKit still red-flags on fresh clone. The corrected shape uses `FRAMEWORK_SEARCH_PATHS[sdk=iphonesimulator*][arch=arm64] = … /iosSimulatorArm64/…` per-arch conditionals, plus README-BOOTSTRAP.md guidance to run `./gradlew :shared:linkDebugFrameworkIosSimulatorArm64` before the first Xcode open on any fresh clone.
