...
0.2 **Every child issue MUST carry:**
   ...
   - An `## ADR Status` block declaring `ADR-trigger: yes|no`. If `yes` → name the specific trigger (e.g., "new external dependency", "new data store", "new transport"), note "architect dispatched FIRST", flag in handoff so task-runner dispatches [[architect]] BEFORE [[implementer]]. If `no` → cite the verbatim `ADR-EXCLUSION` item from the project's `CLAUDE.md` / `AGENT_LOOP.md`. When in doubt → default to `yes`.
